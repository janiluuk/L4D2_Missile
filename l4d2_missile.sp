...
static const Float:MISSILE_TTL = 8.0; // Safety: any fired missile explodes within 8 seconds
...
new Handle:l4d2_missile_push;
new Handle:l4d2_missile_limit;
new Handle:l4d2_missile_kills;
new Handle:l4d2_missile_safe;
new Handle:l4d2_missile_tracefactor;
new Handle:l4d2_missile_radar_range;

new Handle:l4d2_missile_infected_smoker;
new Handle:l4d2_missile_infected_charger;
new Handle:l4d2_missile_infected_spitter;
new Handle:l4d2_missile_infected_witch;
new Handle:l4d2_missile_infected_tank_throw;
new Handle:l4d2_missile_infected_anti;

new Handle:l4d2_missile_weapon_rifle;
new Handle:l4d2_missile_weapon_sniper;
new Handle:l4d2_missile_weapon_shotgun;
new Handle:l4d2_missile_weapon_magnum;
new Handle:l4d2_missile_weapon_smg;
new Handle:l4d2_missile_weapon_pistol;
new Handle:l4d2_missile_weapon_grenade;

// NEW: configurable highlight and cooldown
new Handle:l4d2_missile_highlight;
new Handle:l4d2_missile_cooldown;

new bool:gamestart = false;

new Float:LastUseTime[MAXPLAYERS+1];  // last *successful* missile fire time (cooldown)
...
// NEW: per-missile spawn time so we can guarantee explosion
new Float:MissileSpawnTime[MAXPLAYERS+1];

new Float:PrintTime[MAXPLAYERS+1];
new ShowMsg[MAXPLAYERS+1];
new MissileTargetNotified[MAXPLAYERS+1];
new Float:MissileTargetNotifyTime[MAXPLAYERS+1];
new Float:MissileHighlightTime[MAXPLAYERS+1];

new g_iVelocity;
new bool:L4D2Version;
new g_sprite;

// Forward to let other plugins decide if client can launch missiles at all.
// Does NOT affect highlighting – that’s purely controlled by cvar.
new Handle:g_fwCanLaunchMissile = INVALID_HANDLE;
...
public Plugin:myinfo =
{
    ...
    version = "2.3-coop-versus",
    url = "https://github.com/janiluuk/L4D2_Missile"
}

public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
    // Other plugins can hook this forward and return Plugin_Handled/Plugin_Stop
    // to prevent a given client from launching missiles.
    g_fwCanLaunchMissile = CreateGlobalForward("L4D2Missile_CanLaunch", ET_Event, Param_Cell);
...
public OnPluginStart()
{
    ...
    l4d2_missile_push = CreateConVar("l4d2_missile_push", "1200", "Push force done to target");

    l4d2_missile_safe = CreateConVar("l4d2_missile_safe", "1", "0:Normal chance of damage to survivor, 1:Less chance to hurt survivor [0, 1]");

    l4d2_missile_infected_smoker = CreateConVar("l4d2_missile_infected_smoker", "0.0", "Launch missile when smoker drags [0.0, 30.0]%");
    l4d2_missile_infected_charger = CreateConVar("l4d2_missile_infected_charger", "0.0", "Launch missile when charger charges [0.0, 30.0]%");
    l4d2_missile_infected_spitter = CreateConVar("l4d2_missile_infected_spitter", "0.0", "Launch missile when spitter spits [0.0, 30.0]%");
    l4d2_missile_infected_witch = CreateConVar("l4d2_missile_infected_witch", "0.0", "Launch missile when witch is startled[0.0, 30.0]%");
    l4d2_missile_infected_tank_throw = CreateConVar("l4d2_missile_infected_tank_throw", "0.0", "Launch missile when tank throws rock[0.0, 30.0]%");
    l4d2_missile_infected_anti = CreateConVar("l4d2_missile_infected_anti", "0.0", "Common infected launch missile when survivor launch missile [0.0, 30.0]%");

    l4d2_missile_weapon_rifle = CreateConVar("l4d2_missile_weapon_rifle", "1", "Enable or disable missiles for rifles {0, 1}");
    l4d2_missile_weapon_sniper = CreateConVar("l4d2_missile_weapon_sniper", "1", "Enable or disable missiles for snipers {0, 1}");
    l4d2_missile_weapon_shotgun = CreateConVar("l4d2_missile_weapon_shotgun", "1", "Enable or disable missiles for shotguns {0, 1}");
    l4d2_missile_weapon_magnum = CreateConVar("l4d2_missile_weapon_magnum", "1", "Enable or disable missiles for magnums {0, 1}");
    l4d2_missile_weapon_smg = CreateConVar("l4d2_missile_weapon_smg", "1", "Enable or disable missiles for smgs {0, 1}");
    l4d2_missile_weapon_pistol = CreateConVar("l4d2_missile_weapon_pistol", "1", "Enable or disable missiles for pistols {0, 1}");
    l4d2_missile_weapon_grenade = CreateConVar("l4d2_missile_weapon_grenade", "1", "Enable or disable missiles for grenade launcher {0, 1}");

    l4d2_missile_limit = CreateConVar("l4d2_missile_limit", "3", "Amount of missiles you can carry");
    l4d2_missile_kills = CreateConVar("l4d2_missile_kills", "30", "How many infected killed rewards one missile");
    ...
    // NEW: highlight + cooldown cvars
    l4d2_missile_highlight = CreateConVar("l4d2_missile_highlight", "1", "Highlight shooter for enemy team while homing missile is active. {0, 1}");
    l4d2_missile_cooldown = CreateConVar("l4d2_missile_cooldown", "5.0", "Cooldown in seconds between missile launches per player.");
    ...
    // Commands work in all modes (coop, survival, versus, scavenge)
...
public OnMapStart()
{
    ...
    // fix for the first missile not lagging the server
...
ResetClientState(x)
{
    LastUseTime[x] = 0.0;
    PrintTime[x] = 0.0;
    MissileTargetNotifyTime[x] = 0.0;
    MissileHighlightTime[x] = 0.0;
    ...
    MissileTargetNotified[x] = 0;
    MissileEnemy[x] = 0;
    MissileType[x] = MissileNormal;
    MissileTeams[x] = 0;
    MissileOwner[x] = 0;
    MissileScanTime[x] = 0.0;
    LastTime[x] = 0.0;
    MissileSpawnTime[x] = 0.0;
}
...
Set()
{
    // placeholder for any derived cvar logic
}
...
ShowMissileHelp(client, bool:includeChat)
{
    ...
    new kills = GetConVarInt(l4d2_missile_kills);
    new limit = GetConVarInt(l4d2_missile_limit);
    new Float:cd = GetConVarFloat(l4d2_missile_cooldown);

    PrintHintText(client, "[MISSILES]\nEarn 1 missile per %d kills (max %d)\nCooldown: %.1f s\nGrenade: USE + shoot\nHoming: crouch + USE + shoot", kills, limit, cd);

    if (includeChat)
    {
        PrintToChat(client, "[MISSILES] 1 missile per %d kills (max %d). Cooldown %.1f s. Grenade: USE+shoot. Homing: crouch+USE+shoot.", kills, limit, cd);
    }
}
...
ShowMissileHelpPanel(client)
{
    ...
    if (panel == INVALID_HANDLE) return;

    SetPanelTitle(panel, "Missiles Help");
    DrawPanelText(panel, "How to earn:");
    decl String:line[64];
    Format(line, sizeof(line), "1 missile per %d kills (max %d).", kills, limit);
    DrawPanelText(panel, line);

    Format(line, sizeof(line), "Cooldown between shots: %.1f s.", cd);
    DrawPanelText(panel, line);

    DrawPanelText(panel, "How to fire:");
    DrawPanelText(panel, "- Grenade: hold USE + shoot.");
    DrawPanelText(panel, "- Homing: crouch + USE + shoot.");
    DrawPanelText(panel, "Works in coop, survival, versus and scavenge.");
    DrawPanelItem(panel, "Close");

    SendPanelToClient(panel, client, PanelCloseHandler, 12);
    CloseHandle(panel);
}

public PanelCloseHandler(Handle:menu, MenuAction:action, any:param1, any:param2)
{
    // no-op
}
...
public Action:ability_use(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    if (StrEqual(s, "ability_spit", true))
    {
        ...
        if (IsValidAliveClient(client) && MissileEntity[client] == 0 && GetRandomFloat(0.0, 100.0) < GetConVarFloat(l4d2_missile_infected_spitter))
        {
    ...
    {
        new client = GetClientOfUserId(GetEventInt(event, "userid"));
        if (IsValidAliveClient(client) && !Hooked[client] && GetRandomFloat(0.0, 100.0) < GetConVarFloat(l4d2_missile_infected_tank_throw))
        {
            LaunchMissile(client, missilespeed_trace2, MissileTrace, true, 30.0);
        }
    }
...
public Action:weapon_fire(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    if (GetClientTeam(client) == SurvivorTeam)
    {
        if (GetClientButtons(client) & IN_USE)
        {
            ...
            new Float:cooldown = GetConVarFloat(l4d2_missile_cooldown);
            if (cooldown < 0.1) cooldown = 0.1; // sanity

            if (LastUseTime[client] > 0.0)
            {
                new Float:elapsed = time - LastUseTime[client];
                if (elapsed < cooldown)
                {
                    new Float:remaining = cooldown - elapsed;
                    // Round up to nearest whole second so player can understand
                    new remainingInt = RoundToCeil(remaining);
                    if (remainingInt < 1) remainingInt = 1;
                    PrintHintText(client, "Missile cooldown: %d s left", remainingInt);
                    return Plugin_Continue;
                }
            }
            ...
            if (ok)
            {
                ...
                // StartMissile now returns whether a missile was actually fired
                if (StartMissile(client, time, type))
                {
...
public Action:InfectedAntiMissile(Handle:timer, any:ent)
{
    ...
    for (new client = 1; client <= MaxClients; client++)
    {
        ...
        {
            candidate[index++] = client;
        }
    }
    if (index > 0)
    {
        selected = GetRandomInt(0, index - 1);
        LaunchMissile(candidate[selected], missilespeed_trace2, MissileTrace, true, 30.0);
    }
    return Plugin_Continue;
}
...
UpGrade(x, kill)
{
    ...
    new v = upgradekillcount[x] / GetConVarInt(l4d2_missile_kills);
    upgradekillcount[x] = upgradekillcount[x] % GetConVarInt(l4d2_missile_kills);
    MissileCount[x] += v;
    if (v > 0)
    {
        PrintHintText(x, "Infected killed: %d", totalkillcount[x]);
        if (MissileCount[x] > GetConVarInt(l4d2_missile_limit)) MissileCount[x] = GetConVarInt(l4d2_missile_limit);
        PrintHintText(x, "Missile count: %d", MissileCount[x]);
        if (ShowMsg[x] < 3)
        {
            ShowMissileHelp(x, false);
            ShowMsg[x]++;
        }
    }
}
...
public Action:Event_InfectedDeath(Handle:hEvent, const String:strName[], bool:DontBroadcast)
{
    ...
    {
        if (GetClientTeam(attacker) == SurvivorTeam)
        {
            UpGrade(attacker, 1);
        }
    }
...
public Action:player_death(Handle:hEvent, const String:strName[], bool:DontBroadcast)
{
    ...
    if (attacker > 0 && IsClientInGame(attacker) && GetClientTeam(attacker) == SurvivorTeam)
    {
        if (victim > 0 && IsClientInGame(victim) && GetClientTeam(victim) == InfectedTeam)
        {
            new bool:headshot = GetEventBool(hEvent, "headshot");
            if (headshot) UpGrade(attacker, 5);
            else UpGrade(attacker, 3);
        }
    }

    if (victim > 0)
    {
        // Shooter died before missile hit => destroy missile with no explosion (per previous spec)
...
UnHookMissile(client)
{
    ...
    MissileTargetNotified[client] = 0;
    MissileTargetNotifyTime[client] = 0.0;
    MissileEnemy[client] = 0;
    MissileSpawnTime[client] = 0.0;
}
...
bool:CanClientLaunchMissile(client)
{
    if (!IsValidAliveClient(client)) return false;
    if (!gamestart) return false;

    if (g_fwCanLaunchMissile != INVALID_HANDLE)
    {
        new Action:res = Plugin_Continue;
        Call_StartForward(g_fwCanLaunchMissile);
...
// StartMissile now returns whether a missile was successfully launched
bool:StartMissile(client, Float:time, type = MissileTrace)
{
    time += 0.0;
    if (!CanClientLaunchMissile(client))
    {
        return false;
    }
    ...
    if (MissileCount[client] > 0)
    {
        ...
        if (ok)
        {
            if (GetRandomFloat(0.0, 100.0) < GetConVarFloat(l4d2_missile_infected_anti))
            {
...
bool:LaunchMissile(client, Float:force, type = MissileTrace, bool:up = false, Float:offset)
{
    ...
    MissileSpawnTime[client] = GetEngineTime();
    MissileCount[client] = MissileCount[client] - 1;
    Hooked[client] = true;
    PrintTime[client] = 0.0;
    MissileHighlightTime[client] = 0.0;
    ...
    PrintHintText(client, "Missiles left: %d", MissileCount[client]);
    if (GetClientTeam(client) == InfectedTeam) PrintToChatAll("\x04%N \x03launched a missile.", client);
    return true;
}
...
bool:CreateMissile(client, type, Float:pos[3], Float:vol[3], Float:ang[3])
{
    ...
    if (ok)
    {
        ...
        if (ment > 0 && ok)
        {
            ...
            decl Float:ang2[3];
            decl Float:offset2[3];
            SetVector(offset2, 0.0, 0.0, 80.0);

            NormalizeVector(offset2, offset2);
            ScaleVector(offset2, -0.0);

            AddVectors(pos, offset2, pos);
...
public ThinkMissile(client)
{
    ...
    {
        // Shooter died before rocket hit: just remove missile (no explosion per previous rule)
        UnHookMissile(client);
        return;
    }
    ...
    // TTL: if missile has been in flight too long, force an explosion
    if (MissileSpawnTime[client] > 0.0 && time - MissileSpawnTime[client] > MISSILE_TTL)
    {
        MissileHit(client);
        UnHookMissile(client);
        return;
    ...
    // Highlight shooter for enemies while homing missile is active, if enabled
    if (MissileType[client] == MissileTrace && MissileTeams[client] == SurvivorTeam && GetConVarInt(l4d2_missile_highlight) != 0)
    {
        if (time - MissileHighlightTime[client] > 1.0)
        {
            MissileHighlightTime[client] = time;
            HighlightShooterForTeam(client, InfectedTeam);
        }
    }
...
TraceMissile(client, Float:time, Float:duration)
{
    ...
    new Float:tracefactor = GetConVarFloat(l4d2_missile_tracefactor);
    if (enemy > 0)
    {
        ...
        if (enemy != MissileTargetNotified[client] || time - MissileTargetNotifyTime[client] > TARGET_HINT_COOLDOWN)
        {
            NotifyTargetAndTeam(client, enemy);
            MissileTargetNotified[client] = enemy;
            MissileTargetNotifyTime[client] = time;
        }
        if (show)
        {
            if (enemy > 0 && IsClientInGame(enemy) && IsPlayerAlive(enemy))
            {
                if (client > 0 && IsClientInGame(client) && IsPlayerAlive(client)) PrintHintText(enemy, "MISSILE CHASING YOU! Shooter: %N, distance: %d", client, RoundFloat(disenemy));
                else PrintHintText(enemy, "MISSILE CHASING YOU! Distance: %d", RoundFloat(disenemy));
                EmitSoundToClient(enemy, SOUNDMISSILELOCK);
            }
            if (client > 0 && IsClientInGame(client) && IsPlayerAlive(client))
            {
                if (enemy > 0 && IsClientInGame(enemy) && IsPlayerAlive(enemy)) PrintHintText(client, "Homing missile locked on %N, distance %d", enemy, RoundFloat(disenemy));
                else PrintHintText(client, "Homing missile locked on enemy, distance: %d", RoundFloat(disenemy));
            }
        }
    }
    else if (enemy < 0)
    {
        ...
        decl Float:posenemy2[3];
        GetEntPropVector(MissileEntity[enemy], Prop_Send, "m_vecOrigin", posenemy2);
        ...
        ScaleVector(voffset, modeloffset);
        AddVectors(posenemy2, voffset, posenemy2);

        disenemy = GetVectorDistance(posmissile, posenemy2);
        visible = IfTwoPosVisible(posmissile, posenemy2, MissleModel[enemy], MissileTeam);
        ScaleVector(velocityenemy, duration);
        AddVectors(posenemy2, velocityenemy, posenemy2);
        MakeVectorFromPoints(posmissile, posenemy2, vtrace);
        if (show)
        {
            if (client > 0 && IsClientInGame(client) && IsPlayerAlive(client))
            {
                if (enemy > 0 && IsClientInGame(enemy) && IsPlayerAlive(enemy)) PrintHintText(client, "Homing missile locked on %N, distance: %d", enemy, RoundFloat(disenemy));
                else PrintHintText(client, "Homing missile locked on enemy missile, distance: %d", RoundFloat(disenemy));
            }
        }
        enemyismissile = true;
    }
    if (enemy == 0 && myteam == SurvivorTeam)
    {
        speed = missilespeed_trace2;
        new Float:dis = GetVectorDistance(posmissile, posradar);
        decl Float:posenemy3[3];
        CopyVector(posradar, posenemy3);
        disenemy = dis;
        MakeVectorFromPoints(posmissile, posenemy3, vtrace);
    }
    ...
    }
    {
        ...
        new Float:up2 = CalRay(posmissile, missionangle, -90.0, 0.0, vup, self, print);
    ...
    decl Float:angle2[3];
    GetVectorAngles(newvelocitymissile, angle2);
    if (!L4D2Version) angle2[0] -= 90.0;
    TeleportEntity(ent, NULL_VECTOR, angle2, newvelocitymissile);
...
NotifyTargetAndTeam(shooter, target)
{
    if (!IsValidAliveClient(shooter) || !IsValidAliveClient(target)) return;

    PrintHintText(target, "%N launched a missile at you! Kill them or run!", shooter);

    new targetTeam = GetClientTeam(target);
    NotifyTeamOfMissile(shooter, target, targetTeam);

    if (targetTeam == InfectedTeam && GetConVarInt(l4d2_missile_highlight) != 0)
    {
        HighlightShooterForTeam(shooter, targetTeam);
    }
}
...
NotifyTeamOfMissile(shooter, target, targetTeam)
{
    ...
    {
        if (!IsValidAliveClient(i)) continue;
        if (GetClientTeam(i) != targetTeam) continue;
        if (i == target) continue;
        PrintHintText(i, "%N fired a missile at %N!", shooter, target);
    }
}

HighlightShooterForTeam(shooter, team)
{
    if (!IsValidAliveClient(shooter)) return;
...
MissileHit(client, num = 2)
{
    ...
    if (MissileTeams[client] == InfectedTeam) damage = GetConVarFloat(l4d2_missile_damage_tosurvivor);
    else damage = GetConVarFloat(l4d2_missile_damage);

    new Float:radius = GetConVarFloat(l4d2_missile_radius);
    new Float:pushforce = GetConVarFloat(l4d2_missile_push);
...
ScanEnemy(Float:missilePos[3], Float:radarPos[3], Float:vec[3], enemyteam)
{
    ...
    new Float:range = GetConVarFloat(l4d2_missile_radar_range);
    for (new client = 1; client <= MaxClients; client++)
    {
        if (IsClientInGame(client))
        {
            ...
            if (playerok || ismissile)
            {
                if (ismissile)
                {
                    ...
                    if (enemyteam == SurvivorTeam || GetVectorDistance(enmeyPos, radarPos) < range)
                    {
...
Float:GetSurvivorMinDistance(Float:pos[3])
{
    ...
    for (new client = 1; client <= MaxClients; client++)
    {
        if (IsClientInGame(client) && IsPlayerAlive(client) && GetClientTeam(client) == SurvivorTeam)
        {
...
public bool:DontHitSelfAndSurvivor(entity, mask, any:data)
{
    ...
    {
        if (IsClientInGame(entity) && GetClientTeam(entity) == SurvivorTeam)
        {
            return false;
        }
    }
...
public bool:DontHitSelfAndInfected(entity, mask, any:data)
{
    ...
    {
        if (IsClientInGame(entity) && GetClientTeam(entity) == InfectedTeam)
        {
            return false;
        }
    }
