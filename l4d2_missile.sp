...
public OnPluginStart()
{
	...
	// Always register console commands (coop, survival, versus, scavenge)
	RegConsoleCmd("sm_m", MissileHelp);
	RegConsoleCmd("sm_missilehelp", MissileHelpFull);
	RegConsoleCmd("sm_missiles", MissileHelpFull);

	// Always hook events; internal logic can handle team/mode differences
...
Set()
{
	// placeholder for future cvar-logic; currently nothing dynamic
}
...
ShowMissileHelp(client, bool:includeChat)
{
	...
	PrintHintText(client, "[MISSILES]\nEarn 1 missile per %d kills (max %d)\nGrenade: USE + shoot\nHoming: crouch + USE + shoot", kills, limit);

	if (includeChat)
	{
		PrintToChat(client, "[MISSILES] Earn 1 missile per %d kills (max %d). Grenade: USE + shoot. Homing: crouch + USE + shoot.", kills, limit);
	}
}
...
ShowMissileHelpPanel(client)
{
	...
	DrawPanelText(panel, "Rules:");
	DrawPanelText(panel, "- Works in coop, survival, versus and scavenge.");
	DrawPanelText(panel, "- Target gets a warning and your team sees your lock.");
	DrawPanelText(panel, "- Keep line of sight to maintain locks.");
...
public int PanelCloseHandler(Handle:menu, MenuAction:action, any:param1, any:param2)
{
	return 0;
}
...
public Action:player_death(Handle:hEvent, const String:strName[], bool:DontBroadcast)
{
	...
	if (IsClientInGame(attacker))
	{
		if (GetClientTeam(attacker) == SurvivorTeam)
		{
			...
			{
				if (GetClientTeam(victim) == InfectedTeam)
				{
...
NotifyTargetAndTeam(shooter, target)
{
	...
	PrintHintText(target, "%N launched a missile at you! Take them out or run!", shooter);

	new targetTeam = GetClientTeam(target);
	NotifyTeamOfMissile(shooter, target, targetTeam);

	if (targetTeam == InfectedTeam)
	{
